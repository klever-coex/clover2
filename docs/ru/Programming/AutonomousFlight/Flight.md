...
```python
drone.navigation.navigate_wait(frame_id="map", x=1.0, y=2.0, z=1.5, speed=0.5, yaw=0.0)
```

В примере ниже дрон поднимется на высоту 50 см относительно текущего положения.

```python
drone.navigation.navigate_wait(frame_id="base_link", z=0.5, speed=0.5)
```

Метод `navigate` полностью копирует `navigate_wait`, но не будет ждать, пока дрон долетит до указанной точки.

```python
drone.navigation.navigate(frame_id="map", x=1.0, y=2.0, z=1.5, yaw=0.0, speed=0.5)
```
...
time.sleep(1)
drone.navigation.navigate_wait("base_link", z=1, speed=1.0)

for x, y in square_points:
    time.sleep(1)
    drone.navigation.navigate_wait("base_link", x=x, y=y, speed=0.8)
