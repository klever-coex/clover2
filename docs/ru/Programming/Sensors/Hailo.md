...
Hailo работает с моделями в формате **Hailo Executable Format** (`.hef`). Обычные веса PyTorch (`.pt`) и модели ONNX (`.onnx`) сначала необходимо скомпилировать в HEF на компьютере. Этот процесс описан в разделе [Перевод YOLOv8 в HEF](HailoModel.md).
