...
Основной метод для полёта — функция работает, пока дрон не долетит до указанной точки. Первый аргумент — система координат, относительно которой задаётся точка. В примере ниже дрон полетит в точку 1, 2, 1.5 относительно начала карты со скоростью 0.5 метров в секунду.
```python
