...
`Clover2` — обёртка над ROS 2 Node. Внутри происходит вся магия, создаётся нода и запускает фоновый поток для работы ROS 2.
...
drone.flight_mode()  # режима полёта PX4
```

### Полет по точкам

Основной метод для полета - функция работает пока дрон не долетит до указанной точки. Первый аргумент - система координат относительно которой задется точка. В примере ниже дрон полетит в точку 1, 2, 1.5 относительно начала карты со скоростью 0.5 метров в секунду.
```python
drone.navigate_wait("map", x=1.0, y=2.0, z=1.5, speed=0.5, yaw=0.0)
```

В примере ниже дрон поднимется на высоту 50 см относительно текущего положения.
```python
drone.navigate_wait("base_link", z=0.5, speed=0.5)
```

Метод navigate полностью копирует navigate_wait, но не будет ждать пока дрон долетит до указанной точки.
```python
drone.navigate("map", x=1.0, y=2.0, z=1.5, yaw=0.0, speed=0.5)
```

### Пример: полёт квадратом
