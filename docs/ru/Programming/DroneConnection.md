...
Сейчас удобнее всего работать с терминалом квадрокоптера через Web IDE. Откройте в браузере страницу `http://192.168.11.1:9880`, затем создайте терминал через меню `Terminal` -> `New Terminal`.
...
Также к квадрокоптеру можно подключиться по SSH. В Windows для подключения можно использовать программу PuTTY или команду `ssh` в PowerShell или CMD.
