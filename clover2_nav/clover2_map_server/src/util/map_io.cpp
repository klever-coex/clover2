...
struct convert<clover2_aruco_msgs::msg::Marker> {
    ...
    static Node encode(const clover2_aruco_msgs::msg::Marker& marker) {
        Node node;
        ...
        tf2::Quaternion q(marker.pose.pose.orientation.x,
...
void save_yaml(const std::filesystem::path& filename,
               const clover2_aruco_msgs::msg::MarkerMap& map) {
    YAML::Node root;
    root["name"] = map.name;
    ...
    for (const auto& m : map.markers) {
        YAML::Node entry = YAML::convert<clover2_aruco_msgs::msg::Marker>::encode(m);
        if (common_size > 0.0) {
            entry.remove("size");
        }
    ...
    if (!out.good()) {
        throw std::runtime_error("Unable to open for write: " +
                                 filename.string() + ", reason: " +
                                 strerror(errno));
    }
    out << root;
}
...
map_io::map_io(rclcpp::Logger logger)
    : m_logger(std::move(logger)) {}

void map_io::load(const std::filesystem::path& filename,
                  clover2_aruco_msgs::msg::MarkerMap& map) {
    const std::string ext = filename.extension().string();
    if (ext == ".txt") {
        RCLCPP_DEBUG(m_logger, "Map format by extension: txt (%s)",
                     filename.c_str());
        load_txt(m_logger, filename, map);
    } else if (ext == ".yaml" || ext == ".yml") {
        RCLCPP_DEBUG(m_logger, "Map format by extension: yaml (%s)",
                     filename.c_str());
        load_yaml(m_logger, filename, map);
    } else {
        throw std::runtime_error(
            "Unknown map file extension '" + ext +
            "' (supported: .txt, .yaml, .yml)");
    }
}

void map_io::save(const std::filesystem::path& filename,
                  const clover2_aruco_msgs::msg::MarkerMap& map) const {
    const std::string ext = filename.extension().string();
    if (ext == ".txt") {
        RCLCPP_DEBUG(m_logger, "Saving map as txt (%s)", filename.c_str());
        save_txt(filename, map);
    } else if (ext == ".yaml" || ext == ".yml") {
        RCLCPP_DEBUG(m_logger, "Saving map as yaml (%s)", filename.c_str());
        save_yaml(filename, map);
    } else {
        throw std::runtime_error(
            "Unknown map file extension '" + ext +
            "' (supported: .txt, .yaml, .yml)");
