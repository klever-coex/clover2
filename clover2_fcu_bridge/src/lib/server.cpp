...
#include <clover2/fcu_bridge/backend/fabric.hpp>
#include <clover2/fcu_bridge/offboard.hpp>
#include <clover2/fcu_bridge/server.hpp>
...
void ensure_backend_registered(const std::string& name) {
    const auto backends = clover2::fcu_bridge::offboard::list_backends();
...
server::server(const rclcpp::NodeOptions& options)
    : clover2::common::lifecycle_node("fcu_bridge", options)
...
server::CallbackReturn server::on_configure(
    ...
    try {
        clover2::fcu_bridge::backend::context ctx(*this);
...
server::CallbackReturn server::on_activate(
    ...
    m_state_pub = create_publisher<clover2_fcu_bridge_msgs::msg::State>(
        "~/state", rclcpp::QoS(10));

    m_arm_disarm_srv = create_service<clover2_fcu_bridge_msgs::srv::ArmDisarm>(
        "~/arm_disarm", std::bind(&server::handle_arm_disarm, this, _1, _2),
        rclcpp::ServicesQoS(), m_service_callback_group);

    m_land_srv = create_service<clover2_fcu_bridge_msgs::srv::Land>(
        "~/land", std::bind(&server::handle_land, this, _1, _2),
        rclcpp::ServicesQoS(), m_service_callback_group);

    m_set_position_srv =
        create_service<clover2_fcu_bridge_msgs::srv::SetPosition>(
            "~/set_position",
            std::bind(&server::handle_set_position, this, _1, _2),
            rclcpp::ServicesQoS(), m_service_callback_group);

    m_navigate_srv = create_service<clover2_fcu_bridge_msgs::srv::Navigate>(
    ...
    m_state_publish_timer =
        create_wall_timer(std::chrono::milliseconds(100), [this]() {
            clover2_fcu_bridge_msgs::msg::State state;
...
void server::handle_land(
    [[maybe_unused]] const clover2_fcu_bridge_msgs::srv::Land::Request::
        SharedPtr req,
...
RCLCPP_COMPONENTS_REGISTER_NODE(clover2::fcu_bridge::server)
