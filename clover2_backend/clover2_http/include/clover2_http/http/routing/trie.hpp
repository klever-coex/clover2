...
public:
    ...
    std::pair<HandlerT*, bool> insert(std::span<const std::string> segments,
        ...
        for (const auto& seg : segments) {
            if (!seg.empty() && !is_token_valid(seg)) {
                throw clover2_http::http::core::routing_error(
                    "Invalid path token {} in route {}.", seg, route);
            }
        ...
        if (cur->is_end) {
            return {cur->handler.get(), false};
        }

        cur->is_end = true;
        cur->handler = std::move(handler);
        return {cur->handler.get(), true};
    }

    std::optional<match_result> search(
        std::span<const std::string> segments) const {
        auto cur = root.get();
